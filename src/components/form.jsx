...
export const Form = function () {
  ...
  const handleSubmit = function (e) {
    ...
    if (validName(name)) {
      ...
      inputName.current.style.borderBottom = "1px solid var(--color-gray-05)";
    ...
    if (validEmail(email)) {
      ...
      inputEmail.current.style.borderBottom = "1px solid var(--color-gray-05)";
      iconCrossEmail.current.style.display = "none";
    } else {
      inputEmail.current.style.color = "var(--color-red)";
      inputEmail.current.style.borderBottom = "1px solid var(--color-red)";
      iconCrossEmail.current.style.display = "revert";
    }

    if (validTel(tel)) {
      inputTel.current.style.color = "var(--color-dark-gray)";
      inputTel.current.style.borderBottom = "1px solid var(--color-gray-05)";
    ...
    if (validComp(company)) {
      ...
      inputComp.current.style.borderBottom = "1px solid var(--color-gray-05)";
    ...
    if (!validName(name) || !validEmail(email) || !validTel(tel) || !validComp(company))
      return undefined;
  };
  ...
  const handleDropDown = function () {
    ...
    if (dropdown === "close") {
      ...
      setTimeout(() => (dropDownBox.current.style.visibility = "hidden"), 150);
  ...
  const handleSelect = function ({ target }) {
    [iconCheck1.current, iconCheck2.current, iconCheck3.current].forEach((icon) => {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit} ref={form}>
      ...
      <input className={styles.input} type="text" placeholder="Name" ref={inputName} />
      <input className={styles.input} type="email" placeholder="Email Address" ref={inputEmail} />

      <div className={`${styles.input} ${styles.select}`} onClick={handleDropDown}>
        <p>
        ...
        <img className={styles.iconArrow} src={iconArrowDown} alt="" ref={arrowIcon} />

        <ul className={styles.ul} ref={dropDownBox} data-dropdown="close">
          <li onClick={handleSelect}>
            <span>Basic Pack</span>
            <span className={styles.selectFree}>Free</span>
            <img className={styles.iconCheck} src={iconCheck} ref={iconCheck1} alt="" />
          </li>

          <li onClick={handleSelect}>
            <span>Pro Pack</span>
            <span className={styles.selectFree}>$9.99</span>
            <img className={styles.iconCheck} src={undefined} ref={iconCheck2} alt="" />
          </li>

          <li onClick={handleSelect}>
            <span>Ultimate Pack</span>
            <span className={styles.selectFree}>$19.99</span>
            <img className={styles.iconCheck} src={undefined} ref={iconCheck3} alt="" />
          </li>
        </ul>
      </div>

      <input className={styles.input} type="tel" placeholder="Phone Number" ref={inputTel} />

      <input className={styles.input} type="text" placeholder="Company" ref={inputComp} />

      <input className={styles.btn} type="submit" value="Get on the list" />
    </form>
  );
};
